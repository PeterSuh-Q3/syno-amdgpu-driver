#!/usr/bin/env bash
# Build one DSM 7.4 SPK containing all kernel-4.4.302 platform module bundles.
set -euo pipefail
...
[[ -n "$VERSION" ]] || { echo 'Missing version in spk/INFO' >&2; exit 2; }
...
matrix=$(awk '$1 !~ /^#/ && $2 == "7.4" { print $1 }' "$ROOT/build/ALL-PLATFORMS-kernel4" | sort)
expected=$(printf '%s\n' "${PLATFORMS[@]}" | sort)
[[ "$matrix" == "$expected" ]] || {
  echo 'build/ALL-PLATFORMS-kernel4 does not match all ten supported kernel-4 targets.' >&2
...
WORK=$(mktemp -d "$ROOT/work/driver-universal-k4.XXXXXX")
cleanup() {
...
for platform in "${PLATFORMS[@]}"; do
  asset=$(awk -v p="$platform" -v k="$KERNEL_VERSION" '$1 == p && $2 == k { print $3; found=1 } END { if (!found) exit 1 }' "$ROOT/build/driver-assets.lock") || {
    echo "No pinned $KERNEL_VERSION DRM asset for $platform" >&2
    exit 2
  }
  source=$ASSET_ROOT/$asset
  [[ -s "$source" ]] || { echo "Missing fetched asset: $source" >&2; exit 2; }
  dest=$PAYLOAD/$platform-${DSM_VERSION}-${KERNEL_FLAVOR}
  mkdir -p "$dest"
  install -m 0644 "$source" "$dest/drm-modules.tgz"
  printf 'platform=%s\ndsm=%s\nkernel=%s\nsource=%s\n' \
  ...
  for required in amdgpu.ko i915.ko; do
...
echo "Verified universal kernel-4 SPK: $OUTPUT"
shasum -a 256 "$OUTPUT"
BUILD_OK=1
